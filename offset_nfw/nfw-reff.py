...
class NFWModelReff(object):
    """
    A class that generates effective radius solutions for mis-centered NFW halo profiles.  
    The basic purpose of this class is to generate internal interpolation tables of effective radius,
    to then be used to compute nfw profile. Also it includes direct
    computation of the non-miscentered versions for completeness.
    
    Initializing a class is easy.  
    - You need to set the range of the mis-centering vector, e.g.:
    xm_range = (0.01,100)

    - You do need a cosmology object like those created by astropy,
    since we need to know overdensities.  Once you have one:
    >>>  from offset_nfw import NFWModel
    >>>  nfw_model = NFWModel(cosmology, xm_range=(0.01,100))
