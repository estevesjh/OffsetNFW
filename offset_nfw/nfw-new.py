...
class NFWModel(object):
    ...
    def generate_miscentered_sigma(self, save=True):
        ...
        for i,rmis in enumerate(x):
            ...
            intF = interp1d(np.log(xint), np.log(sigmaMis(xint,rmis)), fill_value='extrapolate')
    ...
    def getProfileVariable(self, kernel, ptype):
        ...
        """
        sigma_profile_dict = {'single': '_miscentered','gamma':'_gamma','_rayleigh'}
        var_name = sigma_profile_dict[kernel]+'_%s_table'%(ptype)
        return getattr(self, var_name)

## Auxialiary Functions
def funcQuadVal(r,rm,func):
...
def sigmaMis(rvec,rmis,eps=1e-5):
    ...
    for i in range(rvec.size):
        res[i] = funcQuadVal(rvec[i],rmis, _sigmaMisQuad)
    return res
...
def f_nfw(x, eps=1e-9):
    ...
    res = np.where(x<1.,f_less_than(x),f_greater_than(x))
    return res
